...
Status: **M0–M3 complete, M4 in progress.** The product requirements are locked (see [`docs/PRD.md`](docs/PRD.md)). `pm-proto` (envelopes, padding, chain derivation, node wire protocol), `pm-crypto` (seed handling, identity derivation, Olm sessions, backup encryption), `pm-store` (SQLCipher-encrypted persistence, Lamport ordering), `pm-transport` (iroh client), `pm-node` (the Server mailbox binary, v0/in-memory), and `pm-core` (the client tying all of the above into `send`/`sync`/backup/restore) are implemented and tested — see each crate's README for what's covered. `pm-ffi` (the uniffi interface exposing `pm-core`) compiles and passes its tests on the host target. `app/` has a React Native turbo-module scaffold wired to it via `uniffi-bindgen-react-native` — see [`app/README.md`](app/README.md) for exactly how far that gets in a Linux/WSL2 environment (iOS is impossible outside macOS; Android cross-compilation currently hits a `rustc` toolchain instability unrelated to this project's code, documented there).
