...
The signed bytes are `bincode::serialize(&(server_addr, updated_at, seq))` — the tuple in that exact order, signature itself never included. Signed with the sender's Ed25519 `signing_key` (§3); verified with `verify_strict` (the non-malleable variant — deliberate, since this gates a security-relevant action) against the sender's known `identity_key`.

Replay/rollback protection is a locally-persisted, monotonically-incrementing counter (`seq`) — deliberately not wall-clock time: an update is accepted only if the signature verifies **and** `seq` is strictly greater than the last `seq` this receiver has already accepted from that sender. `updated_at` still travels alongside it (informational — audit/debugging) but plays no role in the accept/reject decision. Comparing wall-clock timestamps across independent devices isn't reliable enough to gate a security-relevant decision — a genuinely later update can carry an earlier timestamp than one already accepted (device clock drift), which an earlier version of this mechanism couldn't distinguish from an actual replay, silently dropping both identically. `seq` is immune to this by construction, since it never involves comparing two different devices' clocks at all. It's this sender's own local counter (`pm_store::Store::increment_and_get_pointer_broadcast_seq`), included in that device's encrypted backup bundle specifically so restoring onto a new device resumes counting from where it left off rather than restarting at a value every existing contact would then reject.

## 8. Backup encryption

Manual backup export (`docs/PRD.md` §5, "Recovery on a new device") assembles a `BackupBundle` — account pickle, every contact's identity/session/pending-OTK/transport-key state, that contact's full message history, this device's own server address if any, and this device's own mailbox-pointer-update broadcast counter (§7) — bincode-serializes it, pads it (below), then encrypts with **XChaCha20-Poly1305** keyed by `backup_key` (§3). A fresh random 24-byte nonce is generated per encryption and prepended directly to the ciphertext (`nonce || ciphertext`, no separate length prefix needed since the nonce is fixed-size). The same encrypted blob is what a Server mailbox stores verbatim via `PutBackup`/`GetBackup` (§5) — the node never sees the plaintext bundle, only this ciphertext.

**Padding**: unlike an `Envelope` (§2, fixed `[256, 1024, 4096]`-byte buckets — sized for one message, capped at ~4 KB), a backup's plaintext is unbounded (a full message history), so it's padded up to the next **power of two** instead, floored at 4096 bytes, with an 8-byte big-endian length prefix (not `Envelope`'s 4-byte one — a `u32` prefix could in principle silently truncate a large enough backup rather than erroring; `u64` removes the concern entirely). This bounds worst-case overhead to roughly 2× rather than leaving it unbounded, while still ensuring a backup file's size reveals only a coarse power-of-two range, not the exact byte count a naive unpadded encryption would leak — see `docs/THREAT-MODEL.md`.
