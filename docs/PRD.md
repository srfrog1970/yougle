...
- **Basic per-identity rate-limiting on `RegisterSlot`/`Write` is now implemented** (a fixed-window counter keyed by the connecting peer's cryptographically-authenticated endpoint id — see `docs/THREAT-MODEL.md`'s Known gaps section), closing the "no throttling at all" half of this item. What remains open: the original protocol's slot-hash tag-chain scheme (and its associated message-window "stall" behavior) was built specifically to hide contact-graph information from a shared community operator. With no community operator in this design, whether that mechanism is still needed in its original form, or can be simplified, needs re-evaluation at the architecture level — not resolved here. Also still open: no protection against a flood distributed across many distinct identities, only per-identity throttling.
