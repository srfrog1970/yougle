...
- **No rate-limiting or anti-abuse mechanics anywhere in the node protocol.** `RegisterSlot`/`Write` have no per-connection or per-time-window throttling — the only bound present is a flat 1 MiB per-message cap, which limits a single request's size, not request volume over time. Explicitly unresolved (`docs/PRD.md`, Open Items).
...
- **Mailbox-pointer-update rejection can't distinguish a forged update from an honest clock-skew collision.** Replay/rollback protection is a strict `updated_at >` check with no nonce or sequence number; a legitimate update whose timestamp happens to be ≤ the last accepted one (device clock drift, most plausibly) is silently dropped, identically to how an actually-forged update is silently dropped. Not previously documented as a caveat anywhere outside one code comment.
...
- **Whether n0's relay/discovery role warrants its own explicit mitigation or disclosure**, given it's a real third party with visibility into endpoint identities and connection timing across effectively every conversation in the system today — newly surfaced while writing this document, not previously tracked anywhere.
