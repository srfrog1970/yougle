...
- **Resolved: backup files no longer leak an exact size signal.** Backup plaintext is now padded to the next power of two (floored at 4096 bytes) before encryption, the same reason `Envelope`s are padded into fixed buckets (`docs/PROTOCOL.md` §8) — before this, the backup bundle had no equivalent padding, so its encrypted size scaled near-linearly with plaintext size and was a rough proxy for exactly how much history it contained. Honestly scoped: this hides the *exact* byte count, not magnitude entirely — a backup file's size still narrows things down to "somewhere in this power-of-two range" (e.g. 4 KB–8 KB), at up to ~2× space overhead in the worst case. The PRD's own Open Items note that backup file format/storage guidance more broadly (expected size, filename/storage advice) still isn't specified — this closes the specific leak, not that whole open item.
